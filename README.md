...
**Figure 2.** Zebrafish embryo mold design and printing outcome. (A) CAD renderings of two circular mold sizes (20 mm and 13 mm) with slot dimensions in mm. Each slot measures 0.40 mm wide at the narrowest point, 0.60 mm at the widest, and extends 5.50 mm deep; spacing between slots is 1.00 mm, and the slot height above the base is 2.00 mm. The smaller mold was designed for use with 14 mm coverslips, while the larger one was tested with 21 mm coverslips. The mold design was kept circular rather than rectangular to fit tightly within the well and remain level. Design files for both sizes are available for download via the associated GitHub repository. (B) Perspective views of the assembled molds. (C) Exploded view of mold components, with a removable handle to facilitate positioning in small dishes. (D) PrusaSlicer g-code preview illustrating how FDM resolution (0.4 mm nozzle) converts triangular cavities into rectangular slots. (E) Wells in 1% low-melting point agarose made using the seven- and twenty-teeth molds in a 14- and 21- mm glass-bottom dishes, respectively. (F) Left Top. Image showing empty agarose wells created using the seven-teeth mold. Left Bottom. Image showing 56 hpf Tg(fli1a:GFP);Tg gata1a:DsRed) larvae inserted into the wells. The orange rectangle indicates an area magnified in the next panel. Right. Magnified area  of  mounted larvae. Live-imaging of the middle larvae, indicated with asterisk, is illustrated in the next panels (G-I). (G) Brightfield midplane-volume image of the 56hpf larvae Tg(fli1a:GFP);Tg gata1a:DsRed) just prior the onset of time-lapse acquisition. (H) Maximum intensity projection of the first and last timepoint from overnight time-lapse imaging of vasculature, Tg(Fli1a:GFP, cyan), and erythrocytes, Tg(gata1a:DsRed, red), in the developing brain of the mounted larvae. See also Movie S1 (I) High-magnification (63x) imaging of brain vascularization process. Shown is the maximum intensity projection of the first acquisition time point. 

Scale bars F) Left. 1 mm, Right 500 µm; G) 40 µm; H) 50 µm; I) 25 µm.
